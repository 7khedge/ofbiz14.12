...
import org.ofbiz.widget.renderer.fo.*;
...
public class OagisShipmentServices {
            ...
            try {
                    ...
                    if (UtilValidate.isNotEmpty(invItemElementList)) {
                        ...
                        for (Map<String, Object> invitemMap : invitemMapList) {
                            ...
                            for (Element invItemElement : localInvItemElementList) {
                                ...
                                // REFACTOR to use stream(), filter(), mapToDouble(), sum().  Do you spot a possible rounding error in the original code?
                                int totalReserved = 0;
                ...
                if (errorMapList.size() == 0) {
                    ...
                    for (GenericValue shipmentItem : shipmentItemList) {
                        ...
                        int totalItemIssuanceQuantity;
                        List<GenericValue> itemIssuanceList = EntityQuery.use(delegator).from("ItemIssuance").where("shipmentId", shipmentId, "shipmentItemSeqId", shipmentItem.get("shipmentItemSeqId")).queryList();
                        totalItemIssuanceQuantity = itemIssuanceList.stream().mapToInt(itemIssuance -> itemIssuance.getDouble("quantity").intValue()).sum();
    ...
    public static Map<String, Object> oagisSendProcessShipment(DispatchContext ctx, Map<String, Object> context) {
        ...
        if (Debug.infoOn())
            Debug.logInfo("Call to oagisSendProcessShipment for orderId [" + orderId + "], sendToUrl=[" + sendToUrl + "], saveToDirectory=[" + saveToDirectory + "], saveToFilename=[" + saveToFilename + "]", module);
        ...
        try {
            ...
            if (Debug.infoOn())
                Debug.logInfo("Saved OagisMessageInfo for oagisSendProcessShipment message for orderId [" + orderId + "]", module);
            ...
            if (Debug.infoOn())
                Debug.logInfo("Finished rendering oagisSendProcessShipment message for orderId [" + orderId + "]", module);
            ...
            if (Debug.infoOn())
                Debug.logInfo("Message send done for oagisSendProcessShipment for orderId [" + orderId + "], sendToUrl=[" + sendToUrl + "], saveToDirectory=[" + saveToDirectory + "], saveToFilename=[" + saveToFilename + "]", module);
            try {
                Map<String, Object> uomiCtx = FastMap.newInstance();
                uomiCtx.putAll(omiPkMap);
